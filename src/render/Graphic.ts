...
class Graphic {
  ...
  constructor (renderFunction, position = new Vector2D(1, 1), scale = new Vector2D(1, 1), rotation = 0) {
  ...
  rect (x: number, y: number, width: number, height: number, color: string = '#fff') {
    this.context.beginPath()
    this.context.fillStyle = color
